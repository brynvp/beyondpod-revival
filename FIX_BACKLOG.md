...
| **Feed list pull-to-refresh: 2-3 min spinner + no downloads** | `FeedListViewModel.refresh()` called `feedRepository.refreshAllFeeds()` directly — 30 feeds ran serially in the ViewModel coroutine (30-150 sec spinner) and `autoDownloadNewEpisodes()` was never called. **Fix**: route through `FeedUpdateWorker(ALL_FEEDS, IS_MANUAL=true)` exactly like `FeedDetailViewModel`; 1.5 sec spinner, Room Flows update reactively. Also parallelised the worker's ALL_FEEDS loop (Semaphore(6)) so 30 feeds run concurrently instead of serially — expected completion ~10-20s vs ~2 min. | **2026-05-10** |
