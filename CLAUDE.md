...
- **Early Beta (v1.0.10-beta): Backlog suppression applied to manual refresh + cleanup deadlock fixed (2026-05-15). (1) Suppression fix: removed !isManualRefresh from suppression condition — manual refresh now also suppresses when window is full (allDownloaded >= keepCount) and trulyNew == 0. Keeps allDownloaded >= keepCount guard so a partial window (e.g. 1 downloaded, keepCount=5) still fills to N with the next newest episodes. (2) Cleanup deadlock fix: applyRetentionCleanup moved to run BEFORE the suppression return. Previously, when suppression fired it returned early without running cleanup, so an over-full window (e.g. 7 > keepCount=5 from a prior bad session) was never trimmed — stuck forever. Now cleanup always runs; suppression only gates new downloads.**
